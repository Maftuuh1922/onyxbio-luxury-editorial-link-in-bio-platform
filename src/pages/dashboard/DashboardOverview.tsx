    ...
    <div className="p-6 md:p-8 space-y-8 max-w-7xl mx-auto font-karla">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-brand-text flex items-center gap-3">
            Atelier Overview <Sparkles className="w-6 h-6 text-brand-purple opacity-40" />
          </h1>
          <p className="text-brand-muted text-sm font-medium">Insights and growth metrics for your digital legacy.</p>
        </div>
        <div className="flex gap-3">
          <Button variant="outline" className="rounded-xl border-gray-200 hover:bg-white shadow-sm transition-all active:scale-95">Share Profile</Button>
          <Button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((s, i) => (
          ...
          >
            <Card className="bg-white border-none shadow-sm p-6 hover:shadow-xl transition-all group cursor-default relative overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-r from-transparent via-brand-purple/[0.02] to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000" />
              ...
              <p className="text-3xl font-bold text-brand-text group-hover:scale-105 transition-transform origin-left relative z-10">{s.value}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <Card className="lg:col-span-2 bg-white border-none shadow-sm p-8">
          <div className="flex items-center justify-between mb-8">
            ...
            <select className="text-xs font-bold border border-gray-100 bg-gray-50 rounded-lg p-2 focus:ring-2 focus:ring-brand-purple/20 outline-none">
            ...
            {[
              ...
              <div key={i} className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[9px] text-brand-muted/60 mt-1 uppercase tracking-tighter font-bold">{log.time}</p>
                </div>
              </div>
            ))}
            <Button variant="ghost" className="w-full text-brand-purple text-xs font-bold hover:bg-brand-purple/5 mt-4 rounded-xl border border-transparent hover:border-brand-purple/10 transition-all">
