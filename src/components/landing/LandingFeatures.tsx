...
const FeatureSection = ({ 
  ...
  <div id={id} className={bg + " py-24 md:py-32"}>
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className={reverse ? "flex flex-col md:flex-row-reverse gap-16 md:gap-24 items-center" : "flex flex-col md:flex-row gap-16 md:gap-24 items-center"}>
        <motion.div 
        ...
        >
          ...
          <button className="inline-flex items-center gap-2 font-space font-bold text-white hover:gap-4 transition-all">
            Learn more about this <ArrowRight size={20} />
          </button>
        </motion.div>
        <motion.div 
            ...
            <div className="grid grid-cols-2 gap-4">
               {[1,2,3,4].map(i => (
                 <div key={i} className="aspect-square rounded-2xl bg-white/5 border border-white/10 hover:bg-white/15 transition-colors cursor-pointer flex items-center justify-center">
                    <div className={`w-8 h-8 rounded-full ${['bg-[#c9a961]', 'bg-[#a8a8a8]', 'bg-emerald-700', 'bg-red-500'][i-1]}`} />
                 </div>
               ))}
            </div>
          </div>
      ...
      <FeatureSection 
        ...
        visual={
          <div className="relative h-[400px]">
            ...
            <div className="absolute bottom-0 left-0 w-[300px] h-[220px] bg-black rounded-2xl border-4 border-white/20 z-10 shadow-2xl overflow-hidden">
               <div className="bg-white/5 w-full h-full flex items-center justify-center font-space text-white/20">DESKTOP</div>
            </div>
          </div>
        }
      />
