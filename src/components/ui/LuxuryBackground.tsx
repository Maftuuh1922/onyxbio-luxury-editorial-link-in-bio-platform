  ...
  return (
    <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none select-none">
      ...
      <motion.div
        ...
        className="absolute top-[-20%] left-[-10%] w-[80%] h-[80%] blur-[140px] rounded-full opacity-20"
      />
      <motion.div
        animate={{ scale: [1, 1.25, 1], opacity: [0.1, 0.2, 0.1] }}
        transition={{ duration: 20, repeat: Infinity }}
        style={{ backgroundColor: palettePrimary }}
        className="absolute bottom-[-20%] right-[-10%] w-[70%] h-[70%] blur-[140px] rounded-full opacity-10"
      />
      {/* Pattern: Grid */}
      {pattern === 'grid' && (
        <div 
      ...
      {(pattern === 'dust' || pattern === 'constellation') && particles.map((p) => (
        <motion.div
          ...
          style={{
            ...
            boxShadow: `0 0 10px ${palettePrimary}`
          }}
      ...
      {pattern === 'constellation' && (
        <svg className="absolute inset-0 w-full h-full opacity-10">
          <motion.path
            d="M 100 100 L 300 400 L 600 200 Z"
            fill="none"
