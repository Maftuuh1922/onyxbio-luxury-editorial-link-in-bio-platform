  ...
  return (
    <div className="sticky top-24 border border-white/10 rounded-[3rem] p-4 bg-onyx-dark shadow-[0_0_50px_-12px_rgba(201,169,97,0.15)] overflow-hidden aspect-[9/18.5] w-full max-w-[320px] mx-auto group">
      <div className={cn("w-full h-full rounded-[2.5rem] bg-onyx-dark relative overflow-y-auto overflow-x-hidden flex flex-col items-center py-10 px-6 border border-white/5", activeFont.class)}>
        ...
        {/* Identity */}
        <div className="text-center relative z-10 space-y-2 mb-8 w-full">
          <div className="font-ornament text-[8px] tracking-[0.4em] mb-1 opacity-60" style={{ color: activePalette.primary }}>✦ ✦ ✦</div>
          <h2 className="text-xl font-display uppercase tracking-wider" style={{ color: activePalette.primary }}>{name || 'NAME'}</h2>
          <p className="text-[10px] italic text-onyx-white/90">{tagline || 'Your tagline here'}</p>
          {bio && <p className="text-[7px] tracking-[0.2em] uppercase font-ornament text-onyx-gray mt-2 leading-relaxed">{bio}</p>}
        </div>
        {/* Links */}
